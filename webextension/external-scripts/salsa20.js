...
class Salsa20 {
		 ...
		 */
		encrypt(key, message, nonce, counter, options)
		{
		 ...
		 */
		decrypt(key, ciphertext, nonce, counter, options)
		{
			// If the options are unset, set to a blank object
			options = options || {};
		 ...
		 * @returns {Uint8Array|String} Returns an array of bytes or a hexadecimal string
		 */
		generateKeystream(key, length, nonce, counter, options)
		{
			// If the options are unset, set to a blank object
			options = options || {};
			...
			key = this.parseKey(key);
			nonce = this.parseNonce(nonce);
			counter = this.parseCounter(counter, length);

			// Create the keystream
			var keystream = this.generateKeystream(key, length, nonce, counter);

			// If the return type requested is hex, convert the bytes to hex
			if (options.hasOwnProperty('returnType') && (options.returnType === 'hex'))
			{
				return this.bytesToHex(keystream);
			}
			else {
				// By default return a byte array
				return keystream;
				 ...
				 */
				quarterRound(yWord0, yWord1, yWord2, yWord3)
				{
					var sum = null;
					...
					// z1 = y1 ⊕ ((y0 + y3) <<< 7)
					sum = this.sumWords(yWord0, yWord3);
					leftRotation = this.leftRotate(sum, 7);
					zWords[1] = this.xorWords(yWord1, leftRotation);

					// z2 = y2 ⊕ ((z1 + y0) <<< 9)
					sum = this.sumWords(zWords[1], yWord0);
					leftRotation = this.leftRotate(sum, 9);
					zWords[2] = this.xorWords(yWord2, leftRotation);

					// z3 = y3 ⊕ ((z2 + z1) <<< 13)
					sum = this.sumWords(zWords[2], zWords[1]);
					leftRotation = this.leftRotate(sum, 13);
					zWords[3] = this.xorWords(yWord3, leftRotation);

					// z0 = y0 ⊕ ((z3 + z2) <<< 18)
					sum = this.sumWords(zWords[3], zWords[2]);
					leftRotation = this.leftRotate(sum, 18);
					zWords[0] = this.xorWords(yWord0, leftRotation);
				 ...
				 */
				littleEndian(byte0, byte1, byte2, byte3)
				{
				...
				hash(xBytes)
				{
					...
					var resultBytes = new Uint8Array(this.outputByteLength);
					...
					for (var resultIndex = 0, i = 0;  i < 16;  resultIndex += 4, i++)
					{
						...
						var sumWords = this.sumWords(zWords[i], xWords[i]);
				...
				expansion(nonce, key0, key1)
				{
					...
					if (key1.length !== 0)
					{
						...
						this.updateArray(inputBytes, this.constants32[0], 0);	// 4 bytes
						this.updateArray(inputBytes, key0, 4);					// 16 bytes
						this.updateArray(inputBytes, this.constants32[1], 20);	// 4 bytes
						this.updateArray(inputBytes, nonce, 24);				// 16 bytes
						this.updateArray(inputBytes, this.constants32[2], 40);	// 4 bytes
						this.updateArray(inputBytes, key1, 44);				// 16 bytes
						this.updateArray(inputBytes, this.constants32[3], 60);	// 4 bytes
					}
					else {
						// Otherwise use the 16 byte (128 bit key)
						// Salsa20 k(n) = Salsa20(τ0, k, τ1, n, τ2, k, τ3)
						this.updateArray(inputBytes, this.constants16[0], 0);	// 4 bytes
						this.updateArray(inputBytes, key0, 4);					// 16 bytes
						this.updateArray(inputBytes, this.constants16[1], 20);	// 4 bytes
						this.updateArray(inputBytes, nonce, 24);				// 16 bytes
						this.updateArray(inputBytes, this.constants16[2], 40);	// 4 bytes
						this.updateArray(inputBytes, key0, 44);				// 16 bytes
						this.updateArray(inputBytes, this.constants16[3], 60);	// 4 bytes
					}
				 ...
				 */
				encryption(key, message, nonce, counter)
				{
				 ...
				 */
				generateKeystream(key, lengthRequired, nonce, counter)
				{
					// Split the key into separate 16 byte arrays for the expansion function
					var keys = this.splitKey(key);

					// Find number of 64 byte keystream blocks and keystream bytes to create
					var numBlocksToGenerate = Math.ceil(lengthRequired / this.outputByteLength);
					var numKeystreamBytes = numBlocksToGenerate * this.outputByteLength;
					...
					for (var block = 0;  counter < maxCounter;  block += 64, counter++)
					{
						...
						this.updateArray(counterAndNonceBytes, counterBytes, 8);
						...
						// Build output
						this.updateArray(keystreamBytes, expansionBytes, block);
					}
				 ...
				 */
				xorKeystreamAndMessage(keystreamBytes, messageBytes, messageLength)
				{
					...
					return xoredBytes;
						...
						*/
						 ...
						 */
						leftPadding(inputString, padCharacter, totalCharacters)
						{
						 ...
						 * @returns {Number} Returns the resulting decimal word of 32 bits
						 */
						leftRotate(wordA, numBitsToRotate)
						{
							// Store result in 32 bit typed array which will clamp the result to 32 bits
							var wordArray = new Uint32Array(1);
						 ...
						 */
						updateArray(arrayToFill, arrayToFillFrom, startIndex)
						{
						 ...
						 */
						parseCounter(counter, messageLength)
						{
						 ...
						 * @returns {Uint8Array}
						 */
						utf8StringToBytes(unicodeString)
						{
