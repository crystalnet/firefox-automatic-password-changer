...
module.exports = function Recorder() {
    ...
    this.StartRecording = function () {
        ...
        window.addEventListener('submit', onSubmitEventHandler, false);
    ...
    function onSubmitEventHandler(e) {
        ...
        retrieveStep(e.target, false);
    }

    function onClickEventHandler(e) {
        console.log("click event of type " + e.target.type)
        if (e.target.type == "submit") {
            retrieveStep(e.target.form, true);
        }
        else {
    ...
    /**
     *
     * @param form the form on which the event occured
     * @param isClickEvent used to identify from where the call came. Optimally this will be removed if all differences between the
     * event-handlers have been checked if they should be used for both.
     */
    function retrieveStep(form, isClickEvent) {
        if (submitSeen) {
            submitSeen = false;
            return;
        }
    ...
    function storePasswordToFFPWManager(url, username, password, passwordField, usernameField, formSubmitURL) {
        ...
        for (var i = 0; i < pwList.length; i++) {
            console.log("if " + pwList.getItem(i)[2] + " == " + getMainPageFromLink(url) + " && " + pwList.getItem(i)[0] + " == " + username);
        ...
        //TODO: the whole password logic does not belong in the recorder. Move this to a dedicated js file and just transmit the necessary info to it
    ...
    function setMessageValues(m) {
        switch (m[0]) {
            case "BE1":
                ...
                console.log("usernameEmailField was set");
                break;
            case "AP2":
                if (passwordField == "")
                    passwordField = m[4];
                ...
                console.log("actualPasswordField was set");
                break;
            case "NP3":
                ...
                console.log("newPasswordField was set");
                break;
            case "Logout":
                Logout = m;
                console.log("logout is " + m + " now");
                break;
        }
    }
