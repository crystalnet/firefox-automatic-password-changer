...
module.exports = function Recorder(lang) {
    ...
    function onClickEventHandler(event) {
        ...
        if (event.button === 0) {
            ...
            if (clickTempStore !== null)
                userWebPath.setItem(clickTempStore[0], clickTempStore[1]);
            clickTempStore = [orderNumber, ["Click", event.clientX, event.clientY, window.content.innerHeight, window.content.innerWidth, scrollTop, websiteTrunk, "false"]];
            orderNumber++;
        }
    }
    ...
    function registerEventHandlers() {
        ...
        if (ebaySubmitButton !== null) {
            ebaySubmitButton.removeEventListener('click', onClickEventHandler, false);
            ebaySubmitButton.addEventListener('click', onClickEventHandler, false);
        }
        // special case for twitter 'egg' button
        let twitterEggButton = window.content.document.getElementById("user-dropdown-toggle");
        if (twitterEggButton !== null) {
            twitterEggButton.removeEventListener('click', onClickEventHandler, false);
            twitterEggButton.addEventListener('click', onClickEventHandler, false);
        }
        // special case for stargames menu button
        let stargamesButton = window.content.document.querySelector("a[data-gt-name='user-menu-btn']");
        if (stargamesButton !== null) {
            stargamesButton.removeEventListener('click', onClickEventHandler, false);
