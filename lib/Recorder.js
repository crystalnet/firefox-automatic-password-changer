...
module.exports = function Recorder() {
    this.testhook = {
        ...
        injectUserWebPath: function(value){ userWebPath = value; },
        injectWindow: function(value){window = value;},
        injectTabs: function (value) { tabs = value; },
    ...
    function onClickEventHandler(e) {
        if (e.target.type != "submit") {
            ...
            form = e.target.form;
            // TODO: if it truly is a duplicate, change it to a common function
    ...
    function setMessageValues(m) {
        switch (m[0]) {
            ...
            case "AP2":
                if (passwordField == "")
                    passwordField = m[4];
                //TOCHECK: should this be set in the "if"? if not move it to the bottom before break for clarity
