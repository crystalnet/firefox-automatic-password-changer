...
module.exports = function Recorder(lang) {
    ...
    function onClickEventHandler(e) {
        ...
        if (e.button === 0) {
            ...
            if (clickTempStore !== null)
                userWebPath.setItem(clickTempStore[0], clickTempStore[1]);
            clickTempStore = [orderNumber, ["Click", e.clientX, e.clientY, window.innerHeight, window.innerWidth, scrollTop, websiteTrunk, "false"]];
            orderNumber++;
        }
    }
    ...
    this.buildContextMenu = function () {
        recordingMenu = contextMenu.Menu({
            ...
            context: contextMenu.PredicateContext(this.recorderIsActive),
