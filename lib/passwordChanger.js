...
const languageStrings = Translator(LANG.usa);
...
var myRecorder = new Recorder(languageStrings);
...
panel.port.on("stopgorecord", LetStartRecord = function () {
    ...
    if (myRecorder.RecorderIsActive()) {
        ...
        blueprintStorageAccess.saveBlueprint(myRecorder.GetWebPage4PWChange(), myHashtable);
        myHashtable = null;
        ...
        onReady: function onOpen(tab) {
            ...
            AccountlistWorker.port.on("Nav2ChangeForm", function (urlAndName) {
                if (!blueprintStorageAccess.hasBlueprint(urlAndName[0])) {
                    ...
                    var hashtbl = blueprintStorageAccess.getBlueprint(urlAndName[0]);
...
function changeAutomaticPassword(hashtbl, username, url) {
    ...
    const imit = new Imitator(hashtbl, username, url, languageStrings);
