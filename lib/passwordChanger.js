...
var languageStrings = Translator(LANG.usa);
...
function changeAutomaticPassword(hashtbl, username, url){
  ...
  imit = new Imitator(hash4AutPWChange, username, url, languageStrings);
...
function createNewBPFile(bp,url){
  ...
  var split = url.split("//");
  var name = "blueprint_" + split[1] + ".json";//.replace(".","_");
  var path = fileio.join(desktopPath, name);
  ...
  // use 0x02 | 0x10 to open file for appending.
    foStream.init(file, 0x02 | 0x08 | 0x20, 0x0666, 0);
  ...
  converter.close(); // this closes foStream
    window.content.alert(languageStrings["Successfully exported on the desktop as"] + " " + name + "_BP.txt");
}
...
function importNewBlueprint(){
  ...
  savePWCPath(storageKey,newBP);
    window.content.alert(languageStrings["The blueprint was imported.."]);
