...
function openAccountList() {
    ...
    tabs.open({
        ...
        onReady: function onOpen(tab) {
            ...
            AccountlistWorker.port.emit("startBuildingAccountlist", [pwHash, blueprintStorageAccess.getAllBlueprints()]);
                ...
                tabs.open({
                    ...
                    onReady: function onOpen(tab) {
                        let blueprintWorker = tab.attach({
                            contentScriptFile: [self.data.url("external/jquery.min.js"), self.data.url("external/jquery-ui.min.js"), self.data.url("BlueprintContentScript.js")]
                        });

                        blueprintWorker.port.emit("languageStrings", languageStrings);

                        // build Blueprint menu dynamically
                        blueprintWorker.port.emit("startBuildingBlueprints", [blueprintStorageAccess.getAllBlueprints(), pwHash]);
                        blueprintWorker.port.emit("getPasswordList", pwHash);
