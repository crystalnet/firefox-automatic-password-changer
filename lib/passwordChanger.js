        ...
        port.onMessage.addListener(function(message) {
            switch (message.content) {
                case "getLoginDomains":
                    getLoginDomains(port);
                    break;
                case "refreshPasswordList":
                    pwHash = utils.getPasswordList();
                    break;
                case "setPassword":
                    let loginData = message.data;
                    utils.storePasswordToFFPWManager(loginData.formActiveURL, loginData.username, loginData.password, loginData.passwordField, loginData.usernameField, loginData.formSubmitURL, pwHash);
                    break;
            }
        });
    });
...
function getLoginDomains(port) {
    require("sdk/passwords").search({
        onComplete: function (credentials) {
            let domains = [];
            credentials.forEach(function (credential) {
                domains.push(credential.url);
            });
