...
module.exports = function Imitator(obj, username, url, languageStrings) {
            ...
            switch (nextEvent) {
                ...
                case "Click" :
                    ...
                    if (utils.removeTrailingSlash(websiteTrunk) !== utils.removeTrailingSlash(mustWebsiteURL)) {
                        ...
                        if ((hiddenWindow.innerHeight != mustWindowHeight) || (hiddenWindow.innerWidth != mustWindowWidth)) {
    ...
    function changeWindowSize(newHeight, newWidth) {
        hiddenWindow.resizeTo(newWidth, newHeight);
        // to handle size differences due to toolbars or bookmark windows resize again if needed
        let widthDifference = newWidth - hiddenWindow.innerWidth;
        let heightDifference = newHeight - hiddenWindow.innerHeight;
        if(heightDifference !== 0 || widthDifference !== 0){
            hiddenWindow.resizeTo(newWidth + widthDifference, newHeight + heightDifference);
        }
    }
