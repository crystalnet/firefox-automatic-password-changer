...
module.exports = function Imitator(obj, username, url){
	...
	*/
	function changeAlgorithm(websiteChangeDone = false) {
		if(actualStepNum < maxStepNum) {
			...
			switch (nextEvent) {
				case "Input":
					...
					if (utils.removeTrailingSlash(websiteTrunk[0]) != utils.removeTrailingSlash(websiteURL)) {
                        // TODO: Check if this is actually a good idea, could be a problem if URL contains random token
                        if(websiteChangeDone) {
                            // tried changing website, but did not help -> abort
                            console.error("changeAlgo, Input case, websiteChangeDone");
                            stopImitating();
                        } else {
				...
				case "Click" :
                    ...
                    if (utils.removeTrailingSlash(websiteTrunk[0]) != utils.removeTrailingSlash(mustWebsiteURL)) {
						// TODO: Check if this is actually a good idea, could be a problem if URL contains random token
                        if(websiteChangeDone) {
                            // tried changing website, but did not help -> abort
                            console.error("changeAlgo, Click case, websiteChangeDone");
                            stopImitating();
                        } else {
                            changeWebsite(websiteURL);
						}
