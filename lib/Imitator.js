...
module.exports = function Imitator(obj,username, url){
		...
		if(actualStepNum < maxStepNum){
			...
			switch(nextEvent){
				case "SubmitLogin": 
					...
					var formPWFieldName = obj.getItem(actualStepNum)[3];
					var formUsernameFieldName = obj.getItem(actualStepNum)[4];				
					//var nextEventIndex;
					...
					performSubmitLogin(formID, mustWebsiteURL, formAction, formPWFieldName, formUsernameFieldName, oldPassword, username);
				...
				case "SubmitPWChange": 
					...
					performSubmitPWChange(formID, mustWebsiteURL, formAction, oldPassword);
	...
	function performSubmitLogin(formID, formWebsite, formAction, formPWFieldName, formUsernameFieldName, password, username){
		
		worker = tabs.activeTab.attach({
			contentScriptFile: data.url("ModifyingPageContentScript.js")
		});

		console.log("sending submitLoginData to content script: " + formID + " " + formWebsite + " " + formAction + " " + formPWFieldName + " " + formUsernameFieldName + " " + password + " " + username);
		worker.port.emit("submitLoginData", [formID, formWebsite, formAction, formPWFieldName, formUsernameFieldName, password, username]);
	}

	function performSubmitPWChange(formID, formWebsite, formAction, password){
		worker = tabs.activeTab.attach({
			contentScriptFile: data.url("ModifyingPageContentScript.js")
		});

		console.log("sending submitPWChangeData to content script");
		worker.port.emit("submitPWChangeData", [formID, formWebsite, formAction, password, newPassword]);
	}
	...
	function stopImitating(){
		hiddenWin.getBrowser().removeEventListener("DOMContentLoaded",DomContentLoadedEvent,false);
			...
			for(var i = actualStepNum; i < obj.length;i++){
				if(obj.getItem(i)[obj.getItem(i).length] == "SubmitPWChange"){
